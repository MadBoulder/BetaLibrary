...
class WeTransfer(WeTransferBase):
    ...
    def upload_files(self, message, files, filename):
        """
        ...
        :param files: A list of FileStorage objects of files to upload
        :param filename: The name of the file
        :return: The download URL generated by WeTransfer
        """
        files = [File(file_contents, filename) for file_contents in files]
        transfer_id = self.__create_transfer(message, files)
        for file in files:
            part_number = 1
            # with open(file.path, 'rb') as fh:
            parts = funcy.chunks(file.chunk_size, file.file_contents)
            try:
                while True:
                    # bytes_read = file.file.read(file.chunk_size)
                    bytes_read = next(parts)
    ...
    def upload_file(self, message, file_contents, filename):
        """
        Upload a single file.
        :param message: Message
        :param file_contents: file, as a FileStorage, to upload
        :return: The download URL generated by WeTransfer
        """
        return self.upload_files(message, [file_contents], filename)
